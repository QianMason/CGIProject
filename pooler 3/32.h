...
typedef struct { bit32 AorT, GorT, valid; } Primer32;
typedef struct { bit32 MaybeA, MaybeC, MaybeG, MaybeT; } DegeneratePrimer32; /* more than one possibility for each base */
typedef struct {
  union {
    DegeneratePrimer32 D;
    Primer32 notD;
  } p;
  int isD;
} MaybeDegeneratePrimer32;
static MaybeDegeneratePrimer32 wrapPrimer32(Primer32 p) {
  MaybeDegeneratePrimer32 r;
  r.p.notD = p; r.isD = 0; return r;
}
static MaybeDegeneratePrimer32 wrapDegeneratePrimer32(DegeneratePrimer32 p) {
  MaybeDegeneratePrimer32 r;
  r.p.D = p; r.isD = 1; return r;
}
static inline DegeneratePrimer32 upgradeToDegenerate32(MaybeDegeneratePrimer32 p) {
  if(p.isD) return p.p.D;
  DegeneratePrimer32 d;
  Primer32 notD = p.p.notD;
...
static inline bit32 DegenerateValid32(DegeneratePrimer32 p) {
...
static Primer32 parsePrimer32(const char *ACGT) {
  Primer32 r={0,0,0};
  while(1) {
...
static DegeneratePrimer32 parseDegeneratePrimer32(const char *ABC) {
  DegeneratePrimer32 r={0,0,0,0};
  while(1) {
...
static MaybeDegeneratePrimer32 parseMaybeDegeneratePrimer32(const char *ABC) {
  size_t l=strcspn(ABC,"BDHKMNRSVWYbdhkmnrsvwy");
  if(ABC[l]) return wrapDegeneratePrimer32(parseDegeneratePrimer32(ABC));
  else return wrapPrimer32(parsePrimer32(ABC));
}

static inline void PrimerComplement32(Primer32 *p) {
  // A->T, T->A, C->G, G->C.  So AorT stays, GorT flipped.
  p->GorT = ~(p->GorT);
}
static inline void PrimerComplement32D(DegeneratePrimer32 *p) {
  bit32 tmp=p->MaybeA; p->MaybeA=p->MaybeT; p->MaybeT=tmp;
  tmp=p->MaybeG; p->MaybeG=p->MaybeC; p->MaybeC=tmp;
}
static inline void PrimerComplement32MaybeD(MaybeDegeneratePrimer32 *p) {
  if(p->isD) PrimerComplement32D(&(p->p.D));
  else PrimerComplement32(&(p->p.notD));
}

static int NumPossibilities32D_32bases(DegeneratePrimer32 d) {
  /* number of possible primers this degenerate primer might be equal to (assumes aligned right) */
  int r = 1, sR, poss;
  for(sR=0; sR<32; sR++) { /* 32, not sixty four etc, because we want the return value to work with Make2bitFrom32D  */
...
static int NumPossibilities32MaybeD_32bases(MaybeDegeneratePrimer32 d) {
  if(d.isD) return NumPossibilities32D_32bases(d.p.D);
  else return 1;
}
static int Make2bitFrom32D(DegeneratePrimer32 d,ULL *out,ULL *outValid,int possNo,int nPoss) {
  /* note: ULL across all 3 .h variants
     ...
     have to call upgradeToDegenerate32 from the Maybe.
  ...
  */
  ...
  for(sR=0; sR<32; sR++) { /* 32, not sixty four etc */
    ...
    if (poss) {
      ...
      int sL = 62-2*sR; /* IMPORTANT: don't write (32-2) as it'll be changed to 32-2 in 32.h; this is ULL */
      toOut |= (bits << sL);
      toOutValid |= ((ULL)3 << sL);
    } else break;
  } *out=toOut; *outValid = toOutValid;
#define Is_32bit /* will change to Is_32bit in 32.h */
#ifdef Is_32bit
...
static inline Primer32 PrimerReverse32(Primer32 p) {
  /* assumes 'valid' is right-aligned */
  bit32 v=p.valid,i1=p.AorT,i2=p.GorT,o1=0,o2=0;
  while(v) {
...
static inline DegeneratePrimer32 DegeneratePrimerReverse32(DegeneratePrimer32 p) {
  /* assumes right-aligned */
  bit32 i1=p.MaybeA, i2=p.MaybeC, i3=p.MaybeG, i4=p.MaybeT, o1=0, o2=0, o3=0, o4=0;
  while(i1 || i2 || i3 || i4) {
    o1=(o1<<1)|(i1&1); o2=(o2<<1)|(i2&1);
...
static inline MaybeDegeneratePrimer32 MaybeDegeneratePrimerReverse32(MaybeDegeneratePrimer32 p) {
  if(p.isD) return wrapDegeneratePrimer32(DegeneratePrimerReverse32(p.p.D));
  else return wrapPrimer32(PrimerReverse32(p.p.notD));
}

static inline void PrimerTag32(Primer32 *p,Primer32 tag) {
  /* assumes 'valid' is right-aligned in both p and tag */
  int sL = popcount32(p->valid); /* = 32-leading0_32 */
  p->valid |= (tag.valid << sL);
  p->AorT |= (tag.AorT << sL); p->GorT|=(tag.GorT << sL);
}
static inline void DegeneratePrimerTag32(DegeneratePrimer32 *p,DegeneratePrimer32 tag) {
  int sL = popcount32(p->MaybeA | p->MaybeC | p->MaybeG | p->MaybeT);
  p->MaybeA |= (tag.MaybeA << sL);
  p->MaybeC |= (tag.MaybeC << sL);
  p->MaybeG |= (tag.MaybeG << sL);
  p->MaybeT |= (tag.MaybeT << sL);
}
static inline void MaybeDegeneratePrimerTag32(MaybeDegeneratePrimer32 *p,MaybeDegeneratePrimer32 tag) {
  if (tag.isD && !(p->isD)) { /* a degenerate tag (is this possible? well if it is, we're ready...) */
    p->p.D = upgradeToDegenerate32(*p); p->isD = 1;
  }
  if(p->isD) DegeneratePrimerTag32(&(p->p.D),upgradeToDegenerate32(tag));
  else PrimerTag32(&(p->p.notD),tag.p.notD);
}
static inline void PrimerTag32B(Primer32 *p,Primer32 tag) {
  /* for backward primers: reverse the tag first, and add it to the lsb of the primer rather than the msb */
  tag = PrimerReverse32(tag);
  int sL = popcount32(tag.valid);
  p->valid = ((p->valid) << sL) | tag.valid;
  p->AorT = ((p->AorT) << sL) | tag.AorT;
  p->GorT = ((p->GorT) << sL) | tag.GorT;
}
static inline void DegeneratePrimerTag32B(DegeneratePrimer32 *p,DegeneratePrimer32 tag) {
  tag = DegeneratePrimerReverse32(tag);
  int sL = popcount32(tag.MaybeA | tag.MaybeC | tag.MaybeG | tag.MaybeT);
...
static inline void MaybeDegeneratePrimerTag32B(MaybeDegeneratePrimer32 *p,MaybeDegeneratePrimer32 tag) {
  if (tag.isD && !(p->isD)) {
    p->p.D = upgradeToDegenerate32(*p); p->isD = 1;
  }
  if(p->isD) DegeneratePrimerTag32B(&(p->p.D),upgradeToDegenerate32(tag));
  else PrimerTag32B(&(p->p.notD),tag.p.notD);
}
static inline void PrimerRmTag32(Primer32 *p,Primer32 tag) {
  int toRM = popcount32(tag.valid);
  bit32 mask = ~(tag.valid << (popcount32(p->valid) - toRM));
  p->valid &= mask; p->AorT &= mask; p->GorT &= mask;
}
static inline void DegeneratePrimerRmTag32(DegeneratePrimer32 *p,DegeneratePrimer32 tag) {
  bit32 tValid = tag.MaybeA | tag.MaybeC | tag.MaybeG | tag.MaybeT,
    pValid = p->MaybeA | p->MaybeC | p->MaybeG | p->MaybeT;
  int toRM = popcount32(tValid);
  bit32 mask = ~(tValid << (popcount32(pValid) - toRM));
  p->MaybeA &= mask; p->MaybeC &= mask; p->MaybeG &= mask;
  p->MaybeT &= mask;
}
static inline void MaybeDegeneratePrimerRmTag32(MaybeDegeneratePrimer32 *p,MaybeDegeneratePrimer32 tag) {
  if (tag.isD && !(p->isD)) {
    p->p.D = upgradeToDegenerate32(*p); p->isD = 1;
  }
  if(p->isD) DegeneratePrimerRmTag32(&(p->p.D),upgradeToDegenerate32(tag));
  else PrimerRmTag32(&(p->p.notD),tag.p.notD);
}
static inline void PrimerRmTag32B(Primer32 *p,Primer32 tag) {
  int sR = popcount32(tag.valid);
  p->valid >>= sR; p->AorT >>= sR; p->GorT >>= sR;
}
static inline void DegeneratePrimerRmTag32B(DegeneratePrimer32 *p,DegeneratePrimer32 tag) {
  int sR = popcount32(tag.MaybeA | tag.MaybeC | tag.MaybeG | tag.MaybeT);
  p->MaybeA >>= sR; p->MaybeC >>= sR; p->MaybeG >>= sR;
  p->MaybeT >>= sR;
}
static inline void MaybeDegeneratePrimerRmTag32B(MaybeDegeneratePrimer32 *p,MaybeDegeneratePrimer32 tag) {
  if (tag.isD && !(p->isD)) {
    p->p.D = upgradeToDegenerate32(*p); p->isD = 1;
  }
  if(p->isD) DegeneratePrimerRmTag32B(&(p->p.D),upgradeToDegenerate32(tag));
  else PrimerRmTag32B(&(p->p.notD),tag.p.notD);
}

static inline int score32(Primer32 p1,Primer32 p2) {
  /* score the interaction between p1 and p2, fast.
     p2 must have been PrimerReverse32'd by the caller. */
  int sL=(32 - 1/*threshold*/) - leading0_32(p2.valid),
  ...
  Primer32 p1B; /* we start with p1 shifted left */
  p1B.AorT = p1.AorT << sL; p1B.GorT = p1.GorT << sL;
  p1B.valid = p1.valid << sL;
  int reload = sL - leading0_32(p1.valid);
  ...
  while(1) {
    bit32 overlap = p1B.valid & p2.valid;
    if(!overlap) return maxScore; /* all done */
    bit32 bonds = (~(p1B.AorT ^ p2.AorT)) & (p1B.GorT ^ p2.GorT) & overlap;
    int score = 2*popcount32(bonds) - popcount32(overlap);
...
static inline int score32D(DegeneratePrimer32 p1,DegeneratePrimer32 p2) {
  bit32 p1Valid = DegenerateValid32(p1),
    p2Valid = DegenerateValid32(p2);
  int sL=(32 - 1/*threshold*/) - leading0_32(p2Valid),
    maxScore = 0;
  DegeneratePrimer32 p1B;
  p1B.MaybeA = p1.MaybeA << sL;
  p1B.MaybeC = p1.MaybeC << sL;
  p1B.MaybeG = p1.MaybeG << sL;
  p1B.MaybeT = p1.MaybeT << sL;
  bit32 p1Bvalid = p1Valid << sL;
  int reload = sL - leading0_32(p1Valid);
  if(reload<0) reload=0;
  while(1) {
    bit32 overlap = p1Bvalid & p2Valid;
    if(!overlap) return maxScore;
    bit32 bonds = (p1B.MaybeA & p2.MaybeT) |
...
static inline int score32MaybeD(MaybeDegeneratePrimer32 p1,MaybeDegeneratePrimer32 p2) {
  if(p1.isD || p2.isD) return score32D(upgradeToDegenerate32(p1),upgradeToDegenerate32(p2));
  else return score32(p1.p.notD,p2.p.notD);
}

static inline int count32(Primer32 p1,Primer32 p2,
                          int *tried) {
  /* count the number of alignments of >0 bonds,
     for information only.  Similar to score32, but called
...
static inline int count32D(DegeneratePrimer32 p1,DegeneratePrimer32 p2,int *tried) {
  bit32 p1Valid = DegenerateValid32(p1),
    p2Valid = DegenerateValid32(p2);
  int sL=(32 - 1/*threshold*/) - leading0_32(p2Valid),
    count = 0;
...
static inline void count32MaybeD(MaybeDegeneratePrimer32 p1,MaybeDegeneratePrimer32 p2,FILE *f) {
  ...
  int tried = 0;
  int c = ((p1.isD || p2.isD) ? count32D(upgradeToDegenerate32(p1),upgradeToDegenerate32(p2),&tried) : count32(p1.p.notD,p2.p.notD,&tried));
  fprintf(f,"Positions tried: %d\nBonding positions: %d%s\n",tried,c,(c>1)?"  (worst one shown here)":"");
}

static void printBases32(Primer32 p,FILE *f) {
  bit32 i = (bit32)1 << (32-1-leading0_32(p.valid));
  for(; i&p.valid; i>>=1)
    fputc(
         (p.AorT & i) ?
         ((p.GorT & i)?'T':'A') :
...
static void printBases32D(DegeneratePrimer32 p,FILE *f) {
  bit32 valid = DegenerateValid32(p);
  bit32 i = (bit32)1 << (32-1-leading0_32(valid));
  for(; i&valid; i>>=1) {
...
static void printBases32MaybeD(MaybeDegeneratePrimer32 p,FILE *f) {
  if(p.isD) printBases32D(p.p.D,f);
  else printBases32(p.p.notD,f);
}

static void print32_inner(Primer32 p1,int sL,Primer32 p2,bit32 overlap,bit32 bonds,FILE* f) {
  /* code common to print32 and dGprint32 */
  int i1 = leading0_32(p1.valid)-sL, i2 = leading0_32(p2.valid), iMid = leading0_32(overlap); if(i1<i2) { i2-=i1; iMid-=i1; i1=0; } else { i1-=i2; iMid-=i2; i2=0; }
  indent(i1,f); fputs("5'-",f); printBases32(p1,f); fputs("-3'\n",f);
  indent(iMid+(sizeof("5'-")-1), f);
  bit32 bond = (bit32)1 << (32-1-leading0_32(overlap));
  for(; bond&overlap; bond>>=1) fputc((bond&bonds)?'|':'x',f);
  fputc('\n',f); indent(i2,f);
  fputs("3'-",f); printBases32(p2,f);fputs("-5'\n",f);
}
static void print32D_inner(DegeneratePrimer32 p1,int sL,DegeneratePrimer32 p2,bit32 overlap,bit32 bonds,FILE* f) {
      int i1 = leading0_32(DegenerateValid32(p1))-sL, i2 = leading0_32(DegenerateValid32(p2)), iMid = leading0_32(overlap); if(i1<i2) { i2-=i1; iMid-=i1; i1=0; } else { i1-=i2; iMid-=i2; i2=0; }
      indent(i1,f); fputs("5'-",f); printBases32D(p1,f); fputs("-3'\n",f);
      indent(iMid+(sizeof("5'-")-1), f);
      bit32 bond = (bit32)1 << (32-1-leading0_32(overlap));
      for(; bond&overlap; bond>>=1) fputc((bond&bonds)?'|':'x',f);
      fputc('\n',f); indent(i2,f);
      fputs("3'-",f); printBases32D(p2,f); fputs("-5'\n",f);
}
static void print32(Primer32 p1,Primer32 p2,int maxScore,FILE *f) {
  /* maxScore has been found by score32; print a representation
     of the interaction, along with the score */
  int sL=(32 - 1) - leading0_32(p2.valid);
  int sR = 0; Primer32 p1B;
  while(1) {
    if(sL) {
      p1B.AorT = p1.AorT << sL; p1B.GorT = p1.GorT << sL;
      p1B.valid = p1.valid << sL;
      ...
      /* this function is allowed to be a bit slower than
         score32, and we need to keep all bits */
    ...
    if(score == maxScore) {
      ...
      fprintf(f,"Matches = %d\n",popcount32(bonds));
      fprintf(f,"Score = %d\n",maxScore);
      print32_inner(p1,sL-sR,p2,overlap,bonds,f);
...
static void print32D(DegeneratePrimer32 p1,DegeneratePrimer32 p2,int maxScore,FILE *f) {
  int sL=(32 - 1) - leading0_32(DegenerateValid32(p2));
  int sR = 0; DegeneratePrimer32 p1B;
  while(1) {
    if(sL) {
      p1B.MaybeA = p1.MaybeA << sL;
      p1B.MaybeC = p1.MaybeC << sL;
    ...
    bit32 overlap = DegenerateValid32(p1B) & DegenerateValid32(p2);
    bit32 bonds = (p1B.MaybeA & p2.MaybeT) |
      (p1B.MaybeC & p2.MaybeG) |
      (p1B.MaybeG & p2.MaybeC) |
      (p1B.MaybeT & p2.MaybeA);
    ...
    if(score == maxScore) {
      ...
      fprintf(f,"Matches = %d\n",popcount32(bonds));
      fprintf(f,"Score = %d\n",maxScore);
      print32D_inner(p1,sL-sR,p2,overlap,bonds,f);
      //return; /* comment out to print ALL maxScore matches */
    }
    if(!overlap) return; /* needed if not returning above */
    if(sL) sL--; else sR++;
...
static void print32MaybeD(MaybeDegeneratePrimer32 p1,MaybeDegeneratePrimer32 p2,const char *name1,const char *name2,int maxScore,FILE *f) {
  if(!name1 || !*name1) name1="(no name)";
  if(!name2 || !*name2) name2="(no name)";
  fprintf(f,"%s versus %s\n",name1,name2);
  count32MaybeD(p1,p2,f);
  if(p1.isD || p2.isD) print32D(upgradeToDegenerate32(p1),upgradeToDegenerate32(p2),maxScore,f);
  else print32(p1.p.notD,p2.p.notD,maxScore,f);
  fputc('\n',f);
}

static void parseFASTA32(char *fileData,MaybeDegeneratePrimer32 *buf,MaybeDegeneratePrimer32 *tags,int *whichTag,char* *names) {
  ...
  while(*fileData) {
    ...
    if (*fileData == '>') {
      ...
      fileData[lineEnd] = 0;
      MaybeDegeneratePrimer32 mdp =
        parseMaybeDegeneratePrimer32(fileData);
...
static void counts32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,FILE *f) {
  int i,j;
  int counts[32]={0},maxS = 0;
  for(i=0; i<np; i++) for(j=i; j<np; j++) {
      int score = score32MaybeD(forward[i],backward[j]);
      counts[score]++; if(score>maxS) maxS=score;
    }
  for(i=0; i<=maxS; i++) fprintf(f,"%d\t%d\n",i,counts[i]);
  if(f!=stdout) fclose(f);
}
static int pCounts32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,const int *pools,const int *precalcScores) {
  /* like counts32 but includes combinations only if they're in the same pool + count invalid/overlap scores */
  int i,j;
  int counts[32]={0},maxS = 0, other=0;
  for(i=0; i<np; i++) for(j=i; j<np; j++) if(pools[i]==pools[j]) {
        int score = precalcScores ? *precalcScores++ : score32MaybeD(forward[i],backward[j]);
        if(score<32) {
          counts[score]++; if(score>maxS) maxS=score;
        } else other++;
...
static void printBonds32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,FILE *f,int threshold,char* *names,const int *pools) {
  int i,j;
  ...
  for(i=0; i<np; i++) for(j=i; j<np; j++) {
      if(!pools || pools[i]==pools[j]) {
        ...
        if (score >= threshold) {
          ...
          } else print32MaybeD(forward[i],backward[j],names[i],names[j],score,f); /* fallback: print in any order if can't sort by highest score 1st */
        }
      }
    }
  if(sr) {
    ...
    ScoreRecord *s;
    for(s=sr; s<sr2; s++)
      print32MaybeD(forward[s->i],backward[s->j],names[s->i],names[s->j],s->score,f);
    free(sr);
  }
}

static int* triangle32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np) {
  ...
  if(scores) {
    ...
    for(i=0; i<np; i++) for(j=i; j<np; j++)
        *p++ = (i==j ? 0 /* ignore self-interaction */ :
                score32MaybeD(forward[i],backward[j]));
  } return scores;
}

static inline bit32 rm_unstable_bonds32(bit32 bonds,bit32 overlap) {
  ...
  bit32 lone_1s = bonds & ((~bonds)<<1) & ((~bonds)>>1);
  lone_1s &= (overlap>>1); /* left-hand bit is never got rid of */
  return bonds & ~lone_1s;
}
static inline float deltaG32(Primer32 p1,Primer32 p2,const float* table) {
  /* like score32 but does deltaG instead */
  int sL=(32 - 1/*threshold*/) - leading0_32(p2.valid);
  ...
  while(1) {
    ...
    bit32 bonds = rm_unstable_bonds32((~(p1B.AorT ^ p2.AorT)) & (p1B.GorT ^ p2.GorT) & overlap, overlap);
    int shift = 32-2-leading0_32(bonds); bit32 mask=(bit32)3 << shift,
      maskEnd = (bit32)3 << trail0_32(bonds);
...
static inline float deltaG32D(DegeneratePrimer32 p1,DegeneratePrimer32 p2,const float* table) {
  bit32 p1Valid = DegenerateValid32(p1),
    p2Valid = DegenerateValid32(p2);
  int sL=(32 - 1/*threshold*/) - leading0_32(p2Valid);
  float minDG = INFINITY;
  ...
  while(1) {
    bit32 overlap = p1Bvalid & p2Valid;
    if(!overlap) return minDG;
    bit32 bonds = rm_unstable_bonds32((p1B.MaybeA & p2.MaybeT) |
                                      (p1B.MaybeC & p2.MaybeG) |
                                      (p1B.MaybeG & p2.MaybeC) |
...
static inline float deltaG32MaybeD(MaybeDegeneratePrimer32 p1,MaybeDegeneratePrimer32 p2,const float* table) {
  if(p1.isD || p2.isD) return deltaG32D(upgradeToDegenerate32(p1),upgradeToDegenerate32(p2),table);
  else return deltaG32(p1.p.notD,p2.p.notD,table);
}
static void dGprint32(Primer32 p1,Primer32 p2,float minDG,FILE *f,const float *table) {
  int sL=(32 - 1) - leading0_32(p2.valid);
  int sR = 0; Primer32 p1B;
  while(1) {
    if(sL) {
    ...
    bit32 overlap = p1B.valid & p2.valid;
    bit32 bonds0 = (~(p1B.AorT ^ p2.AorT)) & (p1B.GorT ^ p2.GorT) & overlap,
      bonds = rm_unstable_bonds32(bonds0, overlap);
    int shift = 32-2-leading0_32(bonds); bit32 mask=(bit32)3 << shift,
      maskEnd = (bit32)3 << trail0_32(bonds);
    float dG = table[256+((p1B.AorT & mask)>>(shift+1))]; // init
    for(; mask>=maskEnd; mask>>=1,shift--)
    ...
    if(nearlyEqual(dG,minDG)) {
      fprintf(f,"dG = %.3g\n",dG);
      print32_inner(p1,sL-sR,p2,overlap,bonds0,f);
      return;
    }
    if(sL) sL--; else sR++;
  }
}
static void dGprint32D(DegeneratePrimer32 p1,DegeneratePrimer32 p2,float minDG,FILE *f,const float *table) {
  int sL=(32 - 1) - leading0_32(DegenerateValid32(p2));
  int sR = 0; DegeneratePrimer32 p1B;
  while(1) {
    if(sL) {
      ...
      p1B.MaybeG = p1.MaybeG >> sR;
      p1B.MaybeT = p1.MaybeT >> sR;
      assert(sR < 32); /* if this breaks, check the range of nearlyEqual */
    }
    bit32 overlap = DegenerateValid32(p1B) & DegenerateValid32(p2);
    bit32 bonds0 = (p1B.MaybeA & p2.MaybeT) |
      (p1B.MaybeC & p2.MaybeG) |
      (p1B.MaybeG & p2.MaybeC) |
      (p1B.MaybeT & p2.MaybeA),
      bonds = rm_unstable_bonds32(bonds0,overlap);
    ...
    if(nearlyEqual(dG,minDG)) {
      fprintf(f,"dG = %.3g\n",dG);
      print32D_inner(p1,sL-sR,p2,overlap,bonds0,f);
      return;
    }
    if(sL) sL--; else sR++;
  }
}
static void dGprint32MaybeD(MaybeDegeneratePrimer32 p1,MaybeDegeneratePrimer32 p2,const char *name1,const char *name2,float minDG,FILE *f,const float *table) {
  if(!name1 || !*name1) name1="(no name)";
  if(!name2 || !*name2) name2="(no name)";
  fprintf(f,"%s versus %s\n",name1,name2);
  count32MaybeD(p1,p2,f);
  if(p1.isD || p2.isD) dGprint32D(upgradeToDegenerate32(p1),upgradeToDegenerate32(p2),minDG,f,table);
  else dGprint32(p1.p.notD,p2.p.notD,minDG,f,table);
  fputc('\n',f);
}
static void dGprintBonds32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,FILE *f,float threshold,char* *names,const int *pools,const float *table) {
  ...
  TwoRanges tr=t_iBounds(np);
    ...
    for(j=i; j<np; j++) {
      if(!pools || pools[i]==pools[j]) {
        float dG = deltaG32MaybeD(forward[i],backward[j],table);
        if (dG <= threshold) {
          #if defined(_OPENMP)
          #pragma omp critical
          #endif
          ...
          } else dGprint32MaybeD(forward[i],backward[j],names[i],names[j],dG,f,table);
        }
      }
    }
  }
  if(sr) {
    ...
    for(s=sr; s<sr2; s++) {
      ...
      dGprint32MaybeD(forward[s->i],backward[s->j],names[s->i],names[s->j],s->dG,f,table);
    }
    free(sr);
...
static int* dGtriangle32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,const float *table) {
  ...
  if(scores) 
    ...
    for(i=tr.r[r].start, p=scores+t_offset(np,i,i); i<tr.r[r].end; i++,t_Progress("Precalculating dG... ",tr,np,done,&next),done+=np-i)
      for(j=i; j<np; j++)
        *p++ = (i==j ? 0 : dGbucket(deltaG32MaybeD(forward[i],backward[j],table),0x4000-1));
  } fputs("\rPrecalculating dG... done",stderr); prnSeconds((long)(time(NULL)-start)); fputs("\n",stderr); fflush(stderr);
  return scores;
}
static int dGpCounts32(int np,const int *pools,const int *precalcScores,FILE *f) {
  /* this is a combination of counts32 and pCounts32, for the delta-G variant.  pools can be NULL, but not precalcScores */
...
static void dGsCounts32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,const float *table,FILE *f) {
...
static void pStats32(const MaybeDegeneratePrimer32 *forward,const MaybeDegeneratePrimer32 *backward,int np,const int *pools,const int *precalcScores,FILE *f) {
  /* like pCounts32 but outputs per-pool */
...
static void pStats32dG(int np,const int *pools,const int *precalcScores,FILE *f) {
